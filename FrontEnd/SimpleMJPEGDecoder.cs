...
namespace FrontEnd
{
    ...
    static class SimpleMJPEGDecoder
    {
        ...
        /// <returns></returns>
        public async static Task StartAsync(Action<BitmapImage> action, string url, string login = null, string password = null, CancellationToken? token = null, int chunkMaxSize = 1024, int frameBufferSize = 1024 * 1024)
        {
        ...
        static void ParseBuffer(Action<BitmapImage> action, byte[] frameBuffer, ref int frameIdx, ref bool inPicture, ref byte previous, ref byte current, byte[] streamBuffer, int streamLength)
        {
            var idx = 0;
        loop:
            if (idx < streamLength)
